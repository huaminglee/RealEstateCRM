...
namespace RealEstateCRM.Web.Controllers
{
    public class ClientController : BaseController
    {
        ...
        public JsonResult ListQueryForMobile()
        {
            ...
            string sql = "select top 100 c.*,d1.name as projectname,d2.name as groupname from Clients c join departments d1 on c.projectid=d1.id join departments d2 on c.groupid=d2.id where 1=1";
            int projectid = 0;
        ...
        public JsonResult Delete(int id)
        {
            ...
            if (s.CreateTime < DateTime.Today.AddDays(-2))
            {
                result.success = false;
                result.obj = "不能删除两天前的客户信息";
                return Json(result);
            }
            if (s.Way == "电话中心")
            {
                result.success = false;
                result.obj = "不能删除电话中心客户信息";
                return Json(result);
            }
                ...
                TransactionScope tran = new TransactionScope(TransactionScopeOption.RequiresNew, new TimeSpan(0, 5, 0)))
            {
                foreach (string s in strs)
                {
                    ...
                    if (int.TryParse(s, out clientId))
                    {
                        ...
                        if (action == "invalid")
                        {
                            if (client.State == ClientStateEnum.来访客户)
                            {
                                var q1 =
        ...
        public ActionResult InviteListQuery(int projectid, string dateFrom, string dateTo, FormCollection collection, string type)
        {
            ...
            var list = ClientActivityListView.GetReport(projectid, groupid, date1, date2, collection["client"],collection["isdone"]);
