...
N(f,j,"",null,e,true)+">"+m+"</td>"},F=function(e,f,j,m){j=(parseInt(j,10)-1)*parseInt(m,10)+1+f;return'<td onClick="rowclick('+e+');" role="gridcell" class="ui-state-default jqgrid-rownum" '+N(e,f,j,null,f,true)+">"+j+"</td>"},T=function(e){var f,j=[],m=0,p;for(p=0;p<a.p.colModel.length;p++){f=a.p.colModel[p];if(f.name!=="cb"&&f.name!=="subgrid"&&f.name!=="rn"){j[m]=e=="local"?f.name:e=="xml"||e==="xmlstring"?f.xmlmap||f.name:f.jsonmap||f.name;m++}}return j},J=function(e){var f=a.p.remapColumns;if(!f||!f.length)f=b.map(a.p.colModel,
