...
namespace RealEstateCRM.Web.Controllers
{
    public class CardController : BaseController
    {
        ...
        public JsonResult Delete(int id)
        {
            ...
            if (c.BigTime != null && c.SmallTime != c.BigTime)
            {
                if (c.BigTime < DateTime.Today.AddDays(-1))
                {
                    result.obj = "不能删除2天前的办卡信息";
                    return Json(result);
                }
                else
                {
                    ...
                    Utilities.AddLog(db, c.ClientId, Client.LogClass, "删除大卡记录", check);
            ...
            {
                if (c.SmallTime < DateTime.Today.AddDays(-1))
                {
                    result.obj = "不能删除2天前的办卡信息";
                    return Json(result);
                }
