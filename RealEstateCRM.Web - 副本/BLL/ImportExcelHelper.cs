...
namespace BMS.Web.BLL
{
    public class ImportExcelHelper
    {
        ...
        static public Department FindDepartment(List<Department> departments, string name)
        {
            if (name == "") return null;
            Department dept = departments.Where(o => o.Name == name).FirstOrDefault();
            return dept;
        }
        static public SystemUser FindUser(List<SystemUser> users, string name)
        {
            if (name == "") return null;
            SystemUser user = users.Where(o => o.Name == name).FirstOrDefault();
            return user;
        }
    }
}
